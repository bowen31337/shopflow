...
export async function seedDatabase() {
  try {
    ...
    for (const [idx, cat] of categories.entries()) {
      await db.run(
        'INSERT INTO categories (name, slug, description, parent_id, position) VALUES (?, ?, ?, ?, ?)',
        cat.name, cat.slug, cat.description, cat.parent_id, idx
      );
    }
    ...
    const products = [
      {
        category_id: 2, brand_id: 1, name: 'TechPro Laptop Pro 15', slug: 'techpro-laptop-pro-15',
        description: 'High-performance laptop with 15-inch display, perfect for work and entertainment',
        price: 1299.99, compare_at_price: 1499.99, sku: 'TECH-LAP-001', stock_quantity: 25, is_featured: 1
      },
      {
        category_id: 3, brand_id: 1, name: 'TechPro Smartphone X1', slug: 'techpro-smartphone-x1',
        description: 'Latest smartphone with advanced camera and long battery life',
        price: 899.99, compare_at_price: 999.99, sku: 'TECH-PHN-001', stock_quantity: 50, is_featured: 1
      },
      {
        category_id: 5, brand_id: 2, name: 'StyleMax Cotton T-Shirt', slug: 'stylemax-cotton-tshirt',
        description: 'Comfortable cotton t-shirt in various colors',
        price: 29.99, compare_at_price: null, sku: 'STY-TSH-001', stock_quantity: 100, is_featured: 0
      },
      {
        category_id: 5, brand_id: 2, name: 'StyleMax Denim Jeans', slug: 'stylemax-denim-jeans',
        description: 'Classic denim jeans with modern fit',
        price: 79.99, compare_at_price: 99.99, sku: 'STY-JEN-001', stock_quantity: 60, is_featured: 0
      },
      {
        category_id: 6, brand_id: 2, name: 'StyleMax Summer Dress', slug: 'stylemax-summer-dress',
        description: 'Light and breezy summer dress',
        price: 59.99, compare_at_price: null, sku: 'STY-DRS-001', stock_quantity: 40, is_featured: 0
      },
      {
        category_id: 7, brand_id: 3, name: 'HomeComfort Throw Pillow Set', slug: 'homecomfort-throw-pillow-set',
        description: 'Set of 2 decorative throw pillows',
        price: 39.99, compare_at_price: 49.99, sku: 'HOM-PIL-001', stock_quantity: 75, is_featured: 0
      },
      {
        category_id: 8, brand_id: 4, name: 'SportFlex Yoga Mat', slug: 'sportflex-yoga-mat',
        description: 'Non-slip yoga mat with carrying strap',
        price: 34.99, compare_at_price: null, sku: 'SPT-YOG-001', stock_quantity: 80, is_featured: 1
      },
      {
        category_id: 8, brand_id: 4, name: 'SportFlex Running Shoes', slug: 'sportflex-running-shoes',
        description: 'Lightweight running shoes with excellent cushioning',
        price: 119.99, compare_at_price: 149.99, sku: 'SPT-SHO-001', stock_quantity: 45, is_featured: 0
      },
      {
        category_id: 2, brand_id: 1, name: 'TechPro Wireless Mouse', slug: 'techpro-wireless-mouse',
        description: 'Ergonomic wireless mouse with precision tracking',
        price: 24.99, compare_at_price: null, sku: 'TECH-MOU-001', stock_quantity: 150, is_featured: 0
      },
      {
        category_id: 2, brand_id: 1, name: 'TechPro Bluetooth Headphones', slug: 'techpro-bluetooth-headphones',
        description: 'Noise-cancelling wireless headphones',
        price: 179.99, compare_at_price: 219.99, sku: 'TECH-HDP-001', stock_quantity: 35, is_featured: 1
      },
      {
        category_id: 7, brand_id: 5, name: 'EcoLife Bamboo Cutting Board', slug: 'ecolife-bamboo-cutting-board',
        description: 'Sustainable bamboo cutting board',
        price: 29.99, compare_at_price: null, sku: 'ECO-CUT-001', stock_quantity: 90, is_featured: 0
      },
      {
        category_id: 7, brand_id: 5, name: 'EcoLife Reusable Water Bottle', slug: 'ecolife-reusable-water-bottle',
        description: 'Stainless steel insulated water bottle',
        price: 24.99, compare_at_price: 29.99, sku: 'ECO-BTL-001', stock_quantity: 120, is_featured: 0
      }
    ];
    ...
    for (const product of products) {
      ...
      `, product.category_id, product.brand_id, product.name, product.slug, product.description,
        product.price, product.compare_at_price, product.sku, product.stock_quantity, product.is_featured || 0);
    }
    ...
    for (const img of productImages) {
      await db.run(
        'INSERT INTO product_images (product_id, url, is_primary, position) VALUES (?, ?, ?, ?)',
        img.product_id, img.url, img.is_primary, img.position
      );
    }
    ...
    for (const variant of variants) {
      await db.run(
        ...
        variant.product_id, variant.name, variant.value, variant.stock_quantity
      );
    }
    ...
    await db.run(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `, 'admin@shopflow.com', adminPassword, 'Admin User', 'admin', 1);
    ...
    await db.run(`
      INSERT INTO users (email, password_hash, name, role, email_verified)
      VALUES (?, ?, ?, ?, ?)
    `, 'customer@example.com', customerPassword, 'Test Customer', 'customer', 1);
    ...
    for (const promo of promoCodes) {
      await db.run(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `, promo.code, promo.type, promo.value, promo.min_order_amount, promo.max_uses);
    }
