...
async function testCartQuantityUpdate() {
  ...
  try {
    ...
    const cartButton = await page.$('button:has-text("Cart"), button:has-text("🛒"), [aria-label*="cart"], a[href="/cart"]');
    if (cartButton) {
    ...
    if (productLink) {
      ...
      const addToCartButton = await page.$('button:has-text("Add to Cart"), button:has-text("Add to cart"), .add-to-cart');
      if (addToCartButton) {
