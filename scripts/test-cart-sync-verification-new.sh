...
# Test 3: Logout (clear token)
echo ""
echo "🚪 Step 4: Simulating logout (clearing token)"
TOKEN=""
echo "✅ Token cleared"

# Test 4: Verify cart sync functionality
echo ""
echo "🔄 Step 5: Testing cart sync functionality"

# Check if our sync endpoints are available
echo "   Testing cart endpoints..."

# Test fetch cart without auth
FETCH_RESPONSE=$(curl -s http://localhost:3001/api/cart)
if echo "$FETCH_RESPONSE" | grep -q "401\|Unauthorized"; then
    echo "✅ Unauthenticated access properly blocked"
else
    echo "⚠️  Unauthenticated access should be blocked"
fi

# Test with valid token
FETCH_RESPONSE_AUTH=$(curl -s -H "Authorization: Bearer $TOKEN" http://localhost:3001/api/cart)
if [ -z "$TOKEN" ]; then
    echo "⚠️  Cannot test authenticated endpoints without token"
else
    if echo "$FETCH_RESPONSE_AUTH" | grep -q "items"; then
        echo "✅ Authenticated cart fetch works"
    else
        echo "❌ Authenticated cart fetch failed"
        echo "Response: $FETCH_RESPONSE_AUTH"
    fi
fi

# Test 6: Verify frontend integration
echo ""
echo "🌐 Step 6: Testing frontend integration"
