...
async function testHomepage(page) {
  ...
  try {
    ...
    if (header) {
      result.details.push('✅ Header found');
    } else {
      result.details.push('⚠️  Header not found');
    }

    // Check for navigation links
    const navLinks = await page.$$('[href="/products"], [href="/login"], [href="/cart"]');
    if (navLinks.length > 0) {
      result.details.push(`✅ Found ${navLinks.length} navigation links`);
    } else {
      result.details.push('⚠️  No navigation links found');
    }
